  ...
  if (Data || Wildfire.hasMessages()) {
    ...
    try {
      ...
      if(Wildfire.hasMessages()) {
        ...
        var dumps = Wildfire.getMessages('http://meta.firephp.org/Wildfire/Structure/FirePHP/Dump/0.1');
        if(dumps && dumps.length>0) {
            ...
            for( var index in dumps ) {
                ...
                var item = json_parse(dumps[index]);
                
                Firebug.Console.log(item, this.context, "dir", Firebug.DOMPanel.DirTable);
            }        	
        }	
      }
